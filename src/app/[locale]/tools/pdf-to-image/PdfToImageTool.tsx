    ...
    <div className="space-y-6">
      ...
      {!file ? (
        ...
        >
          ...
          <p className="text-sm font-medium text-white">{t("dropPdfOrClick")}</p>
          <p className="mt-1 text-xs text-gray-500">{t("pdfPagesToPng")}</p>
        ...
        <>
          ...
          {processing && (
            <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-6">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-sm text-gray-400">{t("processingPages", { progress })}</span>
              </div>
          ...
          {pages.length > 0 && (
            <>
              <div className="flex items-center justify-between">
                <p className="text-sm text-white font-medium">{t("pagesExtracted", { count: pages.length })}</p>
                <button
                  onClick={downloadAll}
                  className="flex items-center gap-1 rounded-lg bg-pink-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-pink-700"
                >
                  <Download className="h-3 w-3" /> {t("downloadAllZip")}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                {pages.map((page, idx) => (
                  <div key={idx} className="group relative rounded-xl border border-gray-800 bg-gray-900/50 p-2">
                    <img src={page} alt={t("pageNumber", { number: idx + 1 })} className="w-full rounded-lg" />
                    <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/50 opacity-0 transition-opacity group-hover:opacity-100">
                      ...
                      >
                        <Download className="mr-1 inline h-3 w-3" /> {t("pageNumber", { number: idx + 1 })}
                      </button>
                    </div>
                    <p className="mt-1 text-center text-xs text-gray-500">{t("pageNumber", { number: idx + 1 })}</p>
                  </div>
                ))}
              </div>
            </>
