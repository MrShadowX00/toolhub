...
function HumanToUnix({ unit }: { unit: "s" | "ms" }) {
  ...
  const defaultValue = useRef(toDatetimeLocal(new Date()));
  const [input, setInput] = useState(defaultValue.current);
  ...
  const convert = useCallback(() => {
    ...
    // datetime-local gives a local-time string; parse as local
